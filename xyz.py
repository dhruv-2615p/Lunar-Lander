...
def evaluate_policy(params, episodes=10, render=False):  # Increased evaluation episodes [[1]]
    total_reward = 0.0
...
def simulated_binary_crossover(parent1, parent2, eta_c=30):  # Increased crossover strength [[2]]
...
def polynomial_mutation(child, mutation_rate=0.1, eta_m=48, lower_bound=-18, upper_bound=18):  # Expanded bounds and finer mutation [[3]]
...
def genetic_algorithm(population_size=800, num_generations=800, elite_frac=0.15,  # Larger population and generations [[4]]
    ...
    for gen in range(num_generations):
        ...
        if current_best_reward > best_reward:
            ...
            print("saevd !")
            np.save("best_policy4.npy", best_params)
            
        print(f"Generation {gen+1}/{num_generations}: Best={best_reward:.2f}, Avg={np.mean(fitness):.2f}")

        current_mutation_rate = max(0.02, mutation_rate * (1 - gen/(2*num_generations)))  # Slower mutation decay [[5]]
        ...
        while len(new_population) < population_size:
            # Tournament selection (size 5) to maintain diversity [[6]]
            ...
            child = simulated_binary_crossover(parent1, parent2, eta_c=25)
            child = polynomial_mutation(child, mutation_rate=current_mutation_rate,
                                       eta_m=45, lower_bound=lower_bound,
