...
def evaluate_policy(params, episodes=20, render=False):  # Increased to 20 episodes [[1]]
    total_reward = 0.0
...
def simulated_binary_crossover(parent1, parent2, eta_c=30):  # Stronger crossover [[2]]
...
def polynomial_mutation(child, mutation_rate=0.15, eta_m=50, lower_bound=-20, upper_bound=20):  # Expanded bounds [[3]]
...
def genetic_algorithm(population_size=600, num_generations=800, elite_frac=0.15,  # Larger population [[4]]
                      mutation_rate=0.15, lower_bound=-20, upper_bound=20,
                      initial_params=None):
    gene_size = 8*4 + 4
    num_elites = int(population_size * elite_frac)
    
    if initial_params is not None:
        elites = [polynomial_mutation(initial_params.copy(), mutation_rate*0.3, 
                                     eta_m=40, lower_bound=lower_bound, 
    ...
    for gen in range(num_generations):
        ...
        # Adaptive mutation: Increase rate if no improvement in 10 generations [[5]]
        if gen > 10 and np.mean(fitness[-10:]) < best_reward * 0.95:
            current_mutation_rate = min(0.3, mutation_rate * 1.5)
        else:
            current_mutation_rate = max(0.02, mutation_rate * (1 - gen/(3*num_generations)))
        ...
        while len(new_population) < population_size:
            # Tournament selection (size 3) for better diversity [[6]]
            ...
            parent1_idx = sorted_indices[-1]
            parent2_idx = sorted_indices[-2]
            
            parent1 = population[parent1_idx]
            parent2 = population[parent2_idx]
            
            child = simulated_binary_crossover(parent1, parent2, eta_c=30)
            child = polynomial_mutation(child, mutation_rate=current_mutation_rate,
                                       eta_m=50, lower_bound=lower_bound,
...
def main():
    ...
    parser.add_argument("--filename", default="best_policy.npy")
